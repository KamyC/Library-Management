...
public class LoginController {
    ...
    Object loginCheck(HttpServletRequest request) {
        ...
        if (isAdmin) {
            ...
            res.put("stateCode", "1");
            res.put("msg", "Admin Login Success！");
        } else if (isReader) {
            ReaderCard readerCard = loginService.findReaderCardByReaderId(id);
            request.getSession().setAttribute("readercard", readerCard);
            res.put("stateCode", "2");
            res.put("msg", "Reader Login Success！");
        } else {
            res.put("stateCode", "0");
            res.put("msg", "Password Error！");
        }
        return res;
    }
    ...
    public String reAdminPasswdDo(HttpServletRequest request, String oldPasswd, String newPasswd, String reNewPasswd, RedirectAttributes redirectAttributes) {
        ...
        if (password.equals(oldPasswd)) {
            if (loginService.adminRePassword(id, newPasswd)) {
                redirectAttributes.addFlashAttribute("succ", "Revise Password Success！");
                return "redirect:/admin_repasswd.html";
            } else {
                redirectAttributes.addFlashAttribute("error", "Revise Password Fails！");
                return "redirect:/admin_repasswd.html";
            }
        } else {
            redirectAttributes.addFlashAttribute("error", "Old Password Error！");
            return "redirect:/admin_repasswd.html";
        }
    }
